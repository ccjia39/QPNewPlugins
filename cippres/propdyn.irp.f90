...
integer :: ttestai,ttestaj
integer :: tpestai,tpestaj
integer :: ppestai,ppestaj
integer :: klo, khi, k
...
!!!!$OMP PARALLEL DO  PRIVATE(i,j,rmat,cmat,jsta,ista)  SCHEDULE(DYNAMIC) 
...
!   call interp(klo,tgrid%a,m2ecoup(:,j,i),tgrid%na,time,rmat,cmat)
   ...
   !matintrp(j,i) = dcmplx(rmat,cmat)*exp(-imag*(ttesta(i)-ttesta(j))*time)
...
!   call interp(klo,tgrid%a,m2ecoup(:,j,i),tgrid%na,time,rmat,cmat)
   call splint(klo,khi,timegrid,real(mcoup(:,jsta,ista)),aimag(mcoup(:,jsta,ista)),rmat2intrp(:,jsta,ista),cmat2intrp(:,jsta,ista),ntime,time,rmat,cmat)
   !matintrp(jsta,ista) = dcmplx(rmat,cmat)*exp(-imag*(tpesta(i)-tpesta(j))*time)
...
!   call interp(klo,tgrid%a,m2ecoup(:,jsta,ista),tgrid%na,time,rmat,cmat)
   call splint(klo,khi,timegrid,real(mcoup(:,jsta,ista)),aimag(mcoup(:,jsta,ista)),rmat2intrp(:,jsta,ista),cmat2intrp(:,jsta,ista),ntime,time,rmat,cmat)
   !matintrp(jsta,ista) = dcmplx(rmat,cmat)*exp(-imag*(ppesta(i)-ppesta(j))*time)
...
!   call interp(klo,tgrid%a,m2ecoup(:,jsta,ista),tgrid%na,time,rmat,cmat)
   call splint(klo,khi,timegrid,real(mcoup(:,jsta,ista)),aimag(mcoup(:,jsta,ista)),rmat2intrp(:,jsta,ista),cmat2intrp(:,jsta,ista),ntime,time,rmat,cmat)
   !matintrp(jsta,ista) = dcmplx(rmat,cmat)*exp(-imag*(ttesta(i)-0.5d0*vproj**2.d0-tpesta(j))*time)
...
!   call interp(klo,tgrid%a,m2ecoup(:,jsta,ista),tgrid%na,time,rmat,cmat)
   call splint(klo,khi,timegrid,real(mcoup(:,jsta,ista)),aimag(mcoup(:,jsta,ista)),rmat2intrp(:,jsta,ista),cmat2intrp(:,jsta,ista),ntime,time,rmat,cmat)
   !matintrp(jsta,ista) = dcmplx(rmat,cmat)*exp(-imag*(ttesta(i)-1.d0*vproj**2.d0-ppesta(j))*time)
...
!   call interp(klo,tgrid%a,m2ecoup(:,j,i),tgrid%na,time,rmat,cmat)
   call splint(klo,khi,timegrid,real(mcoup(:,jsta,ista)),aimag(mcoup(:,jsta,ista)),rmat2intrp(:,jsta,ista),cmat2intrp(:,jsta,ista),ntime,time,rmat,cmat)
  ! matintrp(jsta,ista) = dcmplx(rmat,cmat)*exp(-imag*(tpesta(i)+0.5d0*vproj**2.d0-ttesta(j))*time)
...
!   call interp(klo,tgrid%a,m2ecoup(:,j,i),tgrid%na,time,rmat,cmat)
   call splint(klo,khi,timegrid,real(mcoup(:,jsta,ista)),aimag(mcoup(:,jsta,ista)),rmat2intrp(:,jsta,ista),cmat2intrp(:,jsta,ista),ntime,time,rmat,cmat)
   !matintrp(jsta,ista) = dcmplx(rmat,cmat)*exp(-imag*(tpesta(i)-0.5d0*vproj**2.d0-ppesta(j))*time)
...
!   call interp(klo,tgrid%a,m2ecoup(:,jsta,ista),tgrid%na,time,rmat,cmat)
   call splint(klo,khi,timegrid,real(mcoup(:,jsta,ista)),aimag(mcoup(:,jsta,ista)),rmat2intrp(:,jsta,ista),cmat2intrp(:,jsta,ista),ntime,time,rmat,cmat)
   !matintrp(jsta,ista) = dcmplx(rmat,cmat)*exp(-imag*(ppesta(i)+1.d0*vproj**2.d0-ttesta(j))*time)
...
!   call interp(klo,tgrid%a,m2ecoup(:,jsta,ista),tgrid%na,time,rmat,cmat)
   call splint(klo,khi,timegrid,real(mcoup(:,jsta,ista)),aimag(mcoup(:,jsta,ista)),rmat2intrp(:,jsta,ista),cmat2intrp(:,jsta,ista),ntime,time,rmat,cmat)
   !matintrp(jsta,ista) = dcmplx(rmat,cmat)*exp(-imag*(ppesta(i)+0.5d0*vproj**2.d0-tpesta(j))*time)
...
! write(*,*) "Psit(1:2)",Psit(1:2)
 ...
 write(*,*) "nsave_time==",nsave_time
 write(*,*) "(timegrid(ntime)-timegrid(5))/nsave_time",timegrid(ntime),timegrid(5),timegrid(10),nsave_time
 ...
 psi(:) = Psit(:)
! write(*,*) "Psit(1:2)",Psit(1:2)
 !write(1234,'(5000(f12.6,1X))') (cdabs(psi(i))**2,i=1,nsta),sum(cdabs(psi(:))**2)
